        ...
        <div className='mx-auto relative'>
            <div className='relative h-[100vh] grid grid-cols-1 lg:grid-cols-5 xl:grid-cols-3 gap-y-5'>
                <div className='lg:col-span-3 xl:col-span-2'>
                    <img src={item?.image} alt="" className='h-full min-w-full' />
                </div>
                <div className='lg:col-span-2 xl:col-span-1 flex flex-col justify-center h-full gap-y-10 lg:px-20 px-5'>
                    <div className='flex flex-col gap-3'>
                        <p>{item?.design} {item?.design && <BsDot className='text-red-700 inline' />} {item?.date}</p>
                        <p className='text-[40px] font-bold'>{item?.title}</p>
                        <p className='text-[17px] font-medium'>{item?.description}</p>
                    </div>
                    ...
                    <a href={item?.link} target="_blank" rel="noopener noreferrer" className='focus:outline-none'><div className='pb-10 flex items-center gap-2 group lg:cursor-pointer '>
